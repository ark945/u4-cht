...
echo "[2/3] 套引擎 patch"
python "$ROOT/tools/convert_lf.py" "$XU4"
git -C "$XU4" apply --ignore-whitespace "$ROOT/patches/engine/cht-engine.patch"

# Windows 音訊修正(timeBeginPeriod + 連 winmm);套在 cht-engine.patch 之上。
# 若已整合則跳過。
if ! grep -q "timeBeginPeriod" "$XU4/src/xu4.cpp"; then
  git -C "$XU4" apply --ignore-whitespace "$ROOT/patches/engine/win-audio.patch" || true
fi
...
if [ ! -f "$ROOT/assets/cjk_font.bin" ] && [ -f "$NOTO" ]; then
  python "$ROOT/tools/build_cjk_font.py" --font "$NOTO" --index "$NOTO_INDEX" --size 22 --cell 24 \
    --out "$ROOT/assets/cjk_font.bin" --preview "$ROOT/assets/cjk_preview.png"
fi
if [ ! -f "$ROOT/assets/cjk_font_firefly.bin" ] && [ -f "$BSMI" ]; then
  python "$ROOT/tools/build_cjk_font.py" --font "$BSMI" --size 22 --cell 24 \
    --out "$ROOT/assets/cjk_font_firefly.bin"
fi
if [ ! -f "$ROOT/assets/cjk_font_kai.bin" ] && [ -f "$BKAI" ]; then
  python "$ROOT/tools/build_cjk_font.py" --font "$BKAI" --size 22 --cell 24 \
...
if [ ! -f "$XU4/module/render/font/cfont-cjk.txf" ] && [ -f "$NOTO" ]; then
  python "$ROOT/tools/build_cjk_txf.py" --font "$NOTO" --index "$NOTO_INDEX" \
    --atlas "$XU4/module/render/font/cfont.png" \
    --out-txf "$XU4/module/render/font/cfont-cjk.txf"
fi

[ -f "$ROOT/assets/u4_cht.tab" ] || python "$ROOT/tools/build_lookup.py" --out "$ROOT/assets/u4_cht.tab"
...
python "$ROOT/tools/patch_vendor_boron.py" \
